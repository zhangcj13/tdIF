...
from .modules import TCL, MyFloor, ScaledNeuron, StraightThrough, IdentityFloor, MySlipReLU
        ...
        if isActivation(module.__class__.__name__.lower()):
            if hasattr(module, "up"):
                ...
                if t == 0:
                    ...
                    # model._modules[name] = MyFloor(float(t), t)
    return model
...
def replace_relu_by_leakyrelu(model, r=0.1):
    for name, module in model._modules.items():
        if hasattr(module, "_modules"):
            model._modules[name] = replace_relu_by_leakyrelu(module, r)
        if isActivation(module.__class__.__name__.lower()):
            model._modules[name] = nn.LeakyReLU(r)
    return model
...
def replace_identity_by_floor(model, t):
    for name, module in model._modules.items():
        if hasattr(module, "_modules"):
            model._modules[name] = replace_activation_by_floor(module, t)
        if isIdentity(module.__class__.__name__.lower()):
            if hasattr(module, "up"):
                print(module.up.item())
                model._modules[name] = IdentityFloor(module.up.item(), t)
            else:
                model._modules[name] = IdentityFloor(8., t)
    return model