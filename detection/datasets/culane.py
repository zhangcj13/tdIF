...
import detection.utils.culane_metric as culane_metric
import cv2
...
class CULane(BaseDataset):
    ...
    def load_annotations(self):
        ...
        with open(self.list_path) as list_file:
            pbar = tqdm(total=self.num_list)
            for n, line in enumerate(list_file):
                infos = self.load_annotation(line.split())
                self.data_infos.append(infos)

                pbar.update(1)
                pbar.set_description("Loading CULane annotations %d" % n)
    ...
    def evaluate(self, predictions, output_basedir,return_metric=False):
        table=None
