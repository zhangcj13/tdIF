...
class CollectLane(object):
    ...
    def target(self, results):
        ...
        for (end_point, line), r in zip(gt_hm_lane_ends, radius):
            ...
            # ireg=(reg*255).astype(np.uint8)
            # cv2.imwrite('ireg.jpg',ireg[0])
            # ireg_mask=(reg_mask*255).astype(np.uint8)
            # cv2.imwrite('ireg_mask.jpg',ireg_mask[0])

            # print('xx')

        # igt_hm=(gt_hm*255).astype(np.uint8)
        # cv2.imwrite('igt_hm.jpg',igt_hm[0])
