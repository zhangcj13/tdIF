...
class Recorder(object):
    def __init__(self, cfg):
        ...
        self.save_cfg(cfg)
        if HAS_GIT:
            self.cp_projects(self.work_dir)  # for git project
        ...
        self.lr = 0.
        self.total_epoch = self.cfg.epochs 
    ...
    def get_work_dir(self):
        no_date_dir = False
        if hasattr(self.cfg,'no_date_dir'):
            no_date_dir = self.cfg.no_date_dir
        if no_date_dir:
             work_dir = self.cfg.work_dirs
        else:
            now = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
            hyper_param_str = '_lr_%1.0e_b_%d' % (self.cfg.optimizer.lr, self.cfg.batch_size)
            if hasattr(self.cfg,'SNN'):
                if self.cfg.SNN['type'] != 'NONE':
                    hyper_param_str = '_lr_%1.0e_b_%d_ts_%d' % (self.cfg.optimizer.lr, self.cfg.batch_size,self.cfg.SNN['time_step'])
    ...
    def __str__(self):
        ...
        eta_seconds = self.batch_time.global_avg * (self.max_iter - self.step)
        eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
        return recording_state.format(self.epoch, self.total_epoch, self.step, self.lr, loss_state, self.data_time.avg, self.batch_time.avg, eta_string)
