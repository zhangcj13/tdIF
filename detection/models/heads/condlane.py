...
from detection.core.lane import Lane
import numpy as np
import math
import random
...
# from ..layers import MyConvModule as ConvModule
...
class CondLaneHead(nn.Module):
    ...
    def __init__(self,
        ...
        if mask_norm_act:
            ...
            # final_act_cfg = dict(type='MyReLU')
        else:
            final_norm_cfg = None
            final_act_cfg = None
        # mask branch
    ...
    def forward_test(
        ...
        def parse_pos(seeds, batchsize, num_classes, h, w, device):
            ...
            poses = torch.from_numpy(np.array(
                poses, np.int_)).long().to(device).unsqueeze(1)
            return poses
    ...
    def infer_snn(self, inputs, post_process=False, hack_seeds=None,hm_thr=0.5,**kwargs):
        def parse_pos(seeds, batchsize, num_classes, h, w, device):
            pos_list = [[p['coord'], p['id_class'] - 1] for p in seeds]
            poses = []
            for p in pos_list:
        ...
        if not post_process:
            ...
            # with Timer("Elapsed time in ctnet_head: %f"):  # 0.3ms
            z = self.ctnet_head(f_hm)
            h_hm, w_hm = f_hm.size()[2:4]
            h_mask, w_mask = f_mask.size()[2:4]
            hms, params = z['hm'], z['params']

            # with Timer("Elapsed time in two branch: %f"):  # 0.6ms
            mask_branchs = self.mask_branch(f_mask)

            return {'m_hms':hms,'m_params':params,'m_mask_branchs':mask_branchs}
        else:
            assert 'm_hms' in inputs.keys() and 'm_params' in inputs.keys() and 'm_mask_branchs' in inputs.keys()
            hms=inputs['m_hms']
            params=inputs['m_params']
            mask_branchs=inputs['m_mask_branchs']
            m_batchsize,_,h_hm,w_hm = hms.size()
            _,_,h_mask,w_mask = mask_branchs.size()
